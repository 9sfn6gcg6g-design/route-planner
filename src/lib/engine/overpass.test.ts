...
describe('buildOverpassQuery', () => {
  ...
  it('also requests barrier nodes (decision 20)', () => {
    const q = buildOverpassQuery({ lat: 51.4545, lon: -2.5879 }, 1200)
    expect(q).toContain('["barrier"]')
    expect(q).toContain('node(around:1200')
  })
})

describe('parseBarrierNodeIds (decision 20)', () => {
  const body = (elements: unknown[]): unknown => ({ elements })

  it('collects blocking barrier nodes and ignores runnable ones and non-nodes', () => {
    const ids = parseBarrierNodeIds(
      body([
        { type: 'node', id: 1, tags: { barrier: 'kissing_gate' } },
        { type: 'node', id: 2, tags: { barrier: 'gate' } },
        { type: 'node', id: 3, tags: { barrier: 'kerb' } }, // runnable — not blocking
        { type: 'way', id: 4, tags: { barrier: 'gate' } }, // not a node
        { type: 'node', id: 5, tags: { highway: 'crossing' } }, // no barrier
      ]),
    )
    expect([...ids].sort((a, b) => a - b)).toEqual([1, 2])
  })

  it('treats a blocking barrier as passable when an access tag admits feet', () => {
    const ids = parseBarrierNodeIds(
      body([
        { type: 'node', id: 1, tags: { barrier: 'gate', foot: 'yes' } },
        { type: 'node', id: 2, tags: { barrier: 'gate', access: 'permissive' } },
        { type: 'node', id: 3, tags: { barrier: 'gate', access: 'private' } },
      ]),
    )
    expect([...ids]).toEqual([3])
  })

  it('is empty for a body with no barrier nodes or a malformed body', () => {
    expect(parseBarrierNodeIds({ elements: [] }).size).toBe(0)
    expect(parseBarrierNodeIds(null).size).toBe(0)
  })
})
